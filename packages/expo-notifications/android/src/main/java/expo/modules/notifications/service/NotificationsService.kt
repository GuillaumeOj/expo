...
open class NotificationsService : BroadcastReceiver() {
  companion object {
    ...
    fun createNotificationResponseBroadcastIntent(context: Context, intent: Intent?): Intent {
      ...
      val action = extras?.getParcelable<NotificationActionRecord>(NOTIFICATION_ACTION_KEY) ?: unmarshalObject(parcelableCreator<NotificationActionRecord>(), extras?.getByteArray(NOTIFICATION_ACTION_BYTES_KEY))
    ...
    fun getNotificationResponseFromBroadcastIntent(intent: Intent): NotificationResponse {
      ...
      val action = intent.getParcelableExtra<NotificationActionRecord>(NOTIFICATION_ACTION_KEY)
        ?: unmarshalObject(parcelableCreator<NotificationActionRecord>(), intent.getByteArrayExtra(NOTIFICATION_ACTION_BYTES_KEY))
